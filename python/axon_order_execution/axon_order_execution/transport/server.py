"""ZMQ Transport Server - bridges ZMQ messages with AxonExecutionService."""
...
from axon_order_execution.models.order import Order
from axon_order_execution.models.portfolio import AccountSummary, Position
from axon_order_execution.models.messages import Response, Event
...
class ZMQTransportServer:
    """ZMQ server that bridges strategy clients with AxonExecutionService.

    Uses two ZMQ sockets:
    - ROUTER: receives Command messages from strategy DEALER sockets,
              dispatches them to AxonExecutionService, returns Response.
    ...
    async def _dispatch(self, command) -> Response:
        """Dispatch a Command to the appropriate AxonExecutionService method."""
        try:

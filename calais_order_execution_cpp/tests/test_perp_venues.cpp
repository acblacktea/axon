...
using calais::venue::VenueMessageKind;
...
TEST(BinanceBuilder, SignedQueryMatchesTheReferenceHmac) {
  ...
  const std::size_t n = venue::binance::BinanceBuilder::place_order_query(
      buf, sizeof(buf), req, 1786106096123LL);
  ASSERT_GT(n, 0u);

  const std::string query(buf, n);
  EXPECT_EQ(query,
            "symbol=BTCUSDT&side=BUY&type=LIMIT&quantity=2.5&price=64000.5"
            "&timeInForce=GTC&recvWindow=5000&timestamp=1786106096123");

  const std::size_t signed_len = venue::binance::BinanceBuilder::sign_query(
      buf, n, sizeof(buf), "test-secret");
  ASSERT_GT(signed_len, n);
  const std::string signed_query(buf, signed_len);
  EXPECT_NE(signed_query.find("&signature="), std::string::npos);
  // Computed independently with Python's hmac module.
  EXPECT_TRUE(signed_query.ends_with(
      "9300f3cab585ea58aa922ec55e843de330f8e3b23b33d61b88f4deab54122342"))
      << signed_query;
}

TEST(BinanceBuilder, PostOnlyUsesGtx) {
  models::OrderRequest req;
  req.instrument = "BTCUSDT";
  req.order_type = OrderType::kLimit;
  req.amount = *core::Qty::from_string("1");
  req.price = D("100");
  req.post_only = true;
  ...
  EXPECT_NE(std::string(buf, n).find("timeInForce=GTX"), std::string::npos);
}

TEST(BinanceBuilder, PercentEncodesClientOrderIds) {
  // An unencoded '&' would split one parameter into two, so the signature
  // would cover different fields from the ones the venue parses.
  models::OrderRequest req;
  req.instrument = "BTCUSDT";
  ...
  const std::size_t n = venue::binance::BinanceBuilder::place_order_query(
      buf, sizeof(buf), req, 1);
  const std::string q(buf, n);
  EXPECT_NE(q.find("newClientOrderId=a%26b%3Dc%20d"), std::string::npos) << q;
}

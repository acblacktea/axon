...
bool FillStore::add_fill(const models::Fill& fill) {
  ...
  try {
    ...
    CALAIS_LOG_ERROR(log(), "failed to persist fill {}: {}", fill.trade_id, e.what());
  ...
  ++accepted_;
  CALAIS_LOG_INFO(log(), "fill {} order={} {} {} amount={} price={} {} fee={} {}",
  ...
  for (const auto& callback : callbacks_) {
    try {
      ...
      CALAIS_LOG_ERROR(log(), "fill callback threw: {}", e.what());
    }
  }
  return true;
}
...
void PortfolioStore::update_account(const models::AccountSummary& account) {
  ...
  try {
    ...
    CALAIS_LOG_ERROR(log(), "failed to persist account {}/{}: {}", account.exchange,
  ...
  for (const auto& cb : account_callbacks_) {
    try {
      ...
      CALAIS_LOG_ERROR(log(), "account callback threw: {}", e.what());
    }
  }
}
...
void PortfolioStore::update_positions(const std::string& exchange,
  ...
  for (const auto& p : positions) {
    try {
      ...
      CALAIS_LOG_ERROR(log(), "failed to persist position {}: {}", p.instrument,
                       e.what());
  ...
  for (const auto& cb : position_callbacks_) {
    try {
      ...
      CALAIS_LOG_ERROR(log(), "position callback threw: {}", e.what());
    }
  }
}

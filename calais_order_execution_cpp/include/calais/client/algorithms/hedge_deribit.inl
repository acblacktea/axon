...
HedgeDeribitResult hedge_deribit_options(Client& client,
  ...
  CALAIS_LOG_INFO(log, "hedge {}: {} (maker) / {} (taker), total={} batch={}",
  ...
  while (remaining.raw() > 0) {
    ...
    if (acquired.raw() <= 0) {
      ...
      result.error = maker.error.empty()
                         ...
                         : maker.error;
      CALAIS_LOG_ERROR(log, "hedge {}: {}", result.internal_order_id, result.error);
      break;
    }
    ...
    CALAIS_LOG_INFO(log, "batch {} done: acquired {}, remaining {}", result.batches,
                    acquired.to_string(), remaining.to_string());
  }

  CALAIS_LOG_INFO(log, "hedge {} finished: {} batches, maker filled {}, taker sent {}",

...
//
// ORDER-ENTRY endpoints do NOT live here -- they are in the EMS, because the
// latency requirements are completely different. These are the backstop reads
// that run on a timer.
...
class VenueRest {
 public:
  ...
  // Cancel and amend, which the perpetual venues cannot do over their
  // WebSocket in this build. `symbol` is required by Binance, Bybit and OKX
  // and is why the EMS has to look the order up first.
  virtual void cancel_order(const std::string& symbol, const std::string& order_id,
                            StringCallback cb) = 0;
  virtual void amend_order(const std::string& symbol, const std::string& order_id,
                           std::optional<core::Qty> amount,
                           std::optional<core::Price> price, StringCallback cb) = 0;

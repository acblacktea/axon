...
void OrderStore::add_order(models::Order order) {
  ...
  try {
    ...
    AXON_LOG_ERROR(log(), "failed to persist new order {}: {}", id, e.what());
    util::get_metrics().inc_db_write_failure("orders");
  }

  AXON_LOG_INFO(log(), "added order {}: {} {} {}", id, order.instrument,
...
void OrderStore::update_order(models::Order order) {
  ...
  if (existing.has_value() && existing->updated_at > order.updated_at) {
    AXON_LOG_WARN(log(), "dropping stale update for {}: existing={} incoming={}",
  ...
  try {
    ...
    AXON_LOG_ERROR(log(), "failed to persist order {}: {}", id, e.what());
    util::get_metrics().inc_db_write_failure("orders");
  }
  ...
  if (!state_changed || already_notified_terminal) {
    if (already_notified_terminal) {
      ...
      AXON_LOG_WARN(log(), "suppressing duplicate terminal notification for {}",
                      id);
    }
    if (terminal) {
      cache_.erase(id);  // (4)
  ...
  AXON_LOG_INFO(log(), "updated order {}: status={} filled={}/{}", id,
...
void OrderStore::reconcile(const std::vector<models::Order>& orders) {
  AXON_LOG_INFO(log(), "reconciling {} orders from the venue", orders.size());
...
void OrderStore::notify(const models::Order& order) {
  for (const auto& callback : callbacks_) {
    try {
      ...
      AXON_LOG_ERROR(log(), "order update callback threw: {}", e.what());
    }
  }
}

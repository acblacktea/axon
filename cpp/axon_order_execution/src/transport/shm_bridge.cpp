...
void ShmBridge::start(const std::string& directory,
  ...
  for (const auto& name : strategies) {
    ...
    pair.events = std::make_unique<core::ShmRing>(core::ShmRing::create(
        directory + "/axon." + name + ".events", kMaxHotMessageBytes, slot_count));
    pair.commands = std::make_unique<core::ShmRing>(core::ShmRing::create(
        directory + "/axon." + name + ".commands", kMaxHotMessageBytes,
        slot_count));
    ...
    AXON_LOG_INFO(log(), "shm fast path for '{}': {} slots x {} bytes", name,
  ...
  for (auto& [name, pair] : rings_) {
    for (;;) {
      ...
      switch (type) {
        ...
        default:
          AXON_LOG_WARN(log(), "[{}] unexpected hot message type {}", name,

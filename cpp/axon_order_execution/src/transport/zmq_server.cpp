...
void ZmqServer::start(const ZmqConfig& config, CommandHandler handler) {
  ...
  running_ = true;
  AXON_LOG_INFO(log(), "ROUTER bound on {}, PUB bound on {}",
                  config.router_endpoint, config.pub_endpoint);
}
...
void ZmqServer::stop() {
  ...
  impl_->router.reset();
  impl_->pub.reset();
  AXON_LOG_INFO(log(), "ZMQ transport stopped");
}
...
std::size_t ZmqServer::poll() {
  ...
  for (;;) {
    ...
    if (frames.size() < 3) {
      AXON_LOG_WARN(log(), "malformed ROUTER message: {} frames", frames.size());
      continue;
    }
    ...
    if (!command.has_value()) {
      AXON_LOG_WARN(log(), "could not parse a command frame");
      continue;
    }
...
void ZmqServer::ResponseSink::operator()(const Response& response) {
  ...
  if (sent_) {
    AXON_LOG_WARN(log(), "a command was answered twice; dropping the second");
    return;
  }
  sent_ = true;
...
void ZmqServer::publish(const Event& event) {
  ...
  try {
    ...
    AXON_LOG_WARN(log(), "could not publish event: {}", e.what());
  }
}

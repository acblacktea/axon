...
void PostgresWriter::start(const DatabaseConfig& config) {
  ...
  AXON_LOG_INFO(log(), "persistence connected, writer thread started");
}
...
void PostgresWriter::stop() {
  ...
  impl_->conn.reset();
  AXON_LOG_INFO(log(), "persistence stopped");
}
...
void PostgresWriter::Impl::run() {
  while (running.load(std::memory_order_acquire)) {
    ...
    try {
      ...
      AXON_LOG_ERROR(log(), "batch write failed ({} rows): {}", batch.size(),
                       e.what());
      ...
      try {
        ...
        AXON_LOG_ERROR(log(), "reconnect failed: {}", reconnect_error.what());
      }
    }
  }
}

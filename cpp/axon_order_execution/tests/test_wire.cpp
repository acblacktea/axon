...
axon::models::Order sample_order() {
  axon::models::Order o;
...
TEST(Wire, EmptyOptionalsEmitNullRatherThanBeingOmitted) {
  ...
  axon::models::Order o = sample_order();
...
TEST(Wire, CommandRoundTripThroughBytes) {
  ...
  EXPECT_EQ(back->typed(), axon::models::CommandType::kPlaceOrder);
...
TEST(Wire, EventRoundTrip) {
  ...
  EXPECT_EQ(back->typed(), axon::models::EventType::kOrderUpdate);
  EXPECT_TRUE(order_from_json(back->data).has_value());

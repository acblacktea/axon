...
axon::models::OrderRequest sample_request() {
...
TEST(HotMessages, InternalIdCapacityMatchesUuid4Hex) {
  ...
  EXPECT_EQ(kInternalIdCap, axon::models::kInternalIdLength);

  auto req = sample_request();
  req.internal_order_id = axon::models::generate_internal_id();
...
TEST(HotMessages, TravelThroughShmRingIntact) {
  const std::string path =
      "/tmp/axon_hot_msgs_" + std::to_string(::getpid());
...
TEST(HotMessages, PayloadAlignmentAllowsDirectCast) {
  ...
  const std::string path =
      "/tmp/axon_hot_align_" + std::to_string(::getpid());

...
ChaseMakerResult chase_maker_fill(Client& client, const ChaseMakerParams& params) {
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    ...
    if (price.raw() <= 0) {
      AXON_LOG_WARN(log, "invalid price for {}, retrying", params.instrument);
      sleep_a_bit();
      continue;
    }
    ...
    if (!current.has_value()) {
      ...
      if (!current.has_value()) {
        AXON_LOG_WARN(log, "order rejected: {}, retrying", error);
        sleep_a_bit();
        continue;
      }
      AXON_LOG_INFO(log, "placed maker order {}: {} {} {} @ {}",
      ...
      if (amended.has_value()) {
        ...
        AXON_LOG_WARN(log, "could not amend order: {}", error);
      }
    }
    ...
    if (const auto updated = client.get_order(current->order_id, error);
      ...
      if (current->status == models::OrderStatus::kFilled) {
        AXON_LOG_INFO(log, "maker order filled: {}", current->order_id);
  ...
  if (current.has_value()) {
    ...
    AXON_LOG_INFO(log, "cancelled unfilled maker order {}", current->order_id);
  ...
  if (params.taker_fallback && remaining.raw() > 0) {
    AXON_LOG_INFO(log, "chase timed out; crossing for the remaining {}",
                    remaining.to_string());
  ...
  AXON_LOG_INFO(log, "chase ended: filled {}/{} after {}s ({} reprices)",

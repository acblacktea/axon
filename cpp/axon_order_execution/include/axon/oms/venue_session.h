...
// Builds the SECOND connection a venue needs for WebSocket order entry, or
// nullptr when it needs none.
//
//   binance  ws-fapi.binance.com/ws-fapi/v1 -- a different host from the user
//            data stream. No session login: every request carries its own
//            apiKey and signature.
//   bybit    stream.bybit.com/v5/trade -- a different path from the private
//            stream, with its own op:"auth" login.
//   okx      nullptr. `op:"order"` rides the private stream connection that
//            already exists, so there is nothing to build.
//   deribit  nullptr. It has always sent orders on its one connection.
//
// The returned session subscribes to nothing: every inbound frame is an
// order-entry reply and goes to handlers.on_rpc_reply. Because it is otherwise
// silent, it sends its own keepalive so the base class's silence watchdog does
// not mistake an idle trade connection for a dead one.

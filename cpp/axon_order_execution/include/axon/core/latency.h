...
class Timeline {
 ...
 public:
  ...
  AXON_ALWAYS_INLINE void stamp(std::size_t stage) noexcept {
    stamps_[stage] = now_ticks();
  }

  // For a stage whose time was captured elsewhere, e.g. a kernel receive
  // timestamp from SO_TIMESTAMPING.
  AXON_ALWAYS_INLINE void set(std::size_t stage, Ticks t) noexcept {
    stamps_[stage] = t;
  }

  AXON_ALWAYS_INLINE Ticks at(std::size_t stage) const noexcept {
  ...
  AXON_ALWAYS_INLINE Ticks delta(std::size_t from, std::size_t to) const noexcept {
    return stamps_[to] - stamps_[from];
  }

  AXON_ALWAYS_INLINE Ticks total() const noexcept { return delta(0, N - 1); }

...
class Strategy:
    ...
    async def clear(self) -> None:
        ...
        logger.info(f"[S2] Clearing orders and positions for {INSTRUMENT}")

        # Cancel all active orders for this instrument
        active_orders = await self._client.get_active_orders()
        for order in active_orders:
            if order.instrument == INSTRUMENT:
                logger.info(f"[S2] Cancelling order {order.order_id}")
        ...
        for pos in positions:
            if pos.instrument == INSTRUMENT and pos.size != 0:
                ...
                amount = abs(pos.size)
                logger.info(f"[S2] Flattening position: {side.value} {amount} {INSTRUMENT}")
